...
optimizer0     = optim.Adam(filter(lambda p: p.requires_grad, diagnoser.parameters()), lr=lr, weight_decay=weight_decay)
...
optimizer     = optim.Adam(diagnoser.parameters(), lr=lr, weight_decay=weight_decay)
for i in range(epoch):
    ...
    optimizer.zero_grad()
    outputs0 = diagnoser(sen_res)
    outputs1 = diagnoser.embedded_forward(r0)
    loss0    = criterion(outputs0, labels0)
    loss1    = criterion(outputs1, labels1)
    loss     = (loss0/3) + (loss1*2/3)
    loss.backward()
    optimizer.step()
    print("embedded training loss", i, "=", loss.item())
#save model
