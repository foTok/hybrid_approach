...
#fault types
#FAULT = ["amplify", "tma", "tmb", "pseudo_rate", "carrier_rate", "carrier_leak"]
FAULT = ["tma", "tmb", "pseudo_rate", "carrier_rate", "carrier_leak", "amplify"]
#fault parameters
#PARA_BEGIN = [0.1, 0.11, (8.8 * 10**6, 10 * 10**6), 0.01, 0.001, 0.1]
PARA_BEGIN = [0.1, (0.8 * 10**6, 7.3 * 10**6), -0.05, -0.05, 0, -0.1]
...
for fault, para_begin, para_step, para_end in zip(FAULT, PARA_BEGIN, PARA_STEP, PARA_END):
    if isinstance(para_begin, tuple):
        for i in range(int((para_end[0]-para_begin[0])/para_step[0])):
            for j in range(int((para_end[1]-para_begin[1])/para_step[1])):
        ...
        for i in range(int((para_end-para_begin)/para_step)):
...
for f1 in range(len(FAULT)):
    ...
    if isinstance(para_begin1, tuple):
        for i in range(int((para_end1[0]-para_begin1[0])/para_step1[0])):
            for j in range(int((para_end1[1]-para_begin1[1])/para_step1[1])):
                ...
                for k in range(int((para_end2-para_begin2)/para_step2)):
        ...
        for k in range(int((para_end1-para_begin1)/para_step1)):
            ...
            if isinstance(para_begin2, tuple):
                ...
                for i in range(int((para_end2-para_begin2)/para_step2)):
